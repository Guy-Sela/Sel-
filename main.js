...
:                           ..  .   :+++=+++++-         .                       
  ...
  const clockData = {
    'semi-linear': {
      src: 'meditativeclocks/clocks/semi-linear_clock_svg/index.html',
  ...
  (function initWorksClockPreload() {
    ...
    const iframes = Array.from(works.querySelectorAll('iframe.clock-preview-iframe'));
  ...
  // CLIPBOARD
  // ==========================================
  document.addEventListener('copy', () => { state.clipboardEvents++; recordActivity(); scheduleHudRender(); });
  document.addEventListener('paste', () => { state.clipboardEvents++; recordActivity(); scheduleHudRender(); });
  document.addEventListener('cut', () => { state.clipboardEvents++; recordActivity(); scheduleHudRender(); });
  ...
  function updateViewport() {
    ...
    state.orientation = window.innerWidth > window.innerHeight ? 'LAND' : 'PORT';
    scheduleHudRender();
  }
  updateViewport();
  window.addEventListener('resize', () => {
  ...
  */
  ...
  function renderHudNow() {
    ...
    setEl('viewport', state.viewport || `${window.innerWidth}×${window.innerHeight}`);
    setEl('orientation', state.orientation || (window.innerWidth > window.innerHeight ? 'LAND' : 'PORT'));
  ...
  (function initNavObservers() {
    ...
    if (worksSection) {
      ...
      const worksRevealObserver = new IntersectionObserver((entries) => {
  ...
  window.addEventListener('scroll', updateRevealTextVisibilityRaf, { passive: true });
  ...
  window.addEventListener('touchstart', () => { navUserTouching = true; }, { passive: true });
  window.addEventListener('touchend', () => { navUserTouching = false; }, { passive: true });
  window.addEventListener('touchcancel', () => { navUserTouching = false; }, { passive: true });
  ...
  window.addEventListener('scroll', handleStickyNavScrollRaf, { passive: true });
